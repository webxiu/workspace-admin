 ...
 */
var TextareaDialog = function (editorUi, title, url, fn, cancelFn, cancelTitle, w, h, addButtons, noHide, noWrap, applyTitle, helpLink, customButtons) {
  w = w != null ? w : 300;
  h = h != null ? h : 120;
...
ExportDialog.exportFile = function (editorUi, name, format, bg, s, b, dpi) {
  ...
  if (format == "xml") {
    ...
    if (param.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA) {
      // opt.exportGraph(mxUtils.getXml(root));
      // editorUi.hideDialog();
    ...
    var svg = mxUtils.getXml(graph.getSvg(window.GRAPH_BACKGROUND, s, b));
    opt.exportGraph(xml, svg, name);
  }
};
...
var LinkDialog = function (editorUi, initialValue, btnLabel, fn) {
  ...
  // linkInput.style.height = "21px";
  // linkInput.style.backgroundImage = "url('" + Dialog.prototype.clearImage + "')";
  // linkInput.style.backgroundRepeat = "no-repeat";
  // linkInput.style.backgroundPosition = "100% 50%";
  // linkInput.style.padding = "5px 14px 5px 5px";
