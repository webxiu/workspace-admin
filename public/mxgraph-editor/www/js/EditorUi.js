  ...
  if (!graph.standalone) {
    ...
    graph.cellEditor.isStopEditingEvent = function (evt) {
      return (
        ...
        (evt.keyCode == 13 &&
          ((!mxClient.IS_SF && mxEvent.isControlDown(evt)) || (mxClient.IS_MAC && mxEvent.isMetaDown(evt)) || (mxClient.IS_SF && mxEvent.isShiftDown(evt))))
      );
    };
...
EditorUi.prototype.showShapePicker = function (x, y, source, callback, direction) {
  ...
  if (cells != null && cells.length > 0) {
    ...
    var addCell = mxUtils.bind(this, function (cell) {
      ...
      node.style.cssText =
        "position:relative;display:inline-block;position:relative;" + "width:30px;height:30px;cursor:pointer;overflow:hidden;padding:3px 0 0 3px;";
...
EditorUi.prototype.updatePasteActionStates = function () {
  ...
  paste.setEnabled(
    this.editor.graph.cellEditor.isContentEditing() || (!mxClipboard.isEmpty() && graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent()))
  );
...
EditorUi.prototype.updateActionStates = function () {
  ...
  this.actions
    .get("collapsible")
    .setEnabled(vertexSelected && (graph.isContainer(graph.getSelectionCell()) || graph.model.getChildCount(graph.getSelectionCell()) > 0));
...
EditorUi.prototype.saveFile = function (forceDialog) {
  var fileName = this.editor.filename || this.editor.getOrCreateFilename();
  console.log('111', forceDialog ? '另存为' : "保存", fileName);
    ...
    try {
      if (Editor.useLocalStorage) {
        ...
        if (xml.length < MAX_REQUEST_SIZE) {
          ...
          // 保存
          var xml = decodeURIComponent(xml);
          var svg = mxUtils.getXml(this.editor.graph.getSvg(window.GRAPH_BACKGROUND, 1, 0)); // (svg对象需要转xml格式)
          opt.saveGraph(xml, svg, name);
        } else {
 ...
 * 关联单据的弹窗
 */
EditorUi.prototype.showOrderDialog = function (cell) {
  let dataList = [
    {
      avatar: null,
      createDate: "2023-07-12 16:00:00",
      deptId: 9,
      deptName: "系统组",
      groupName: null,
      id: 1384864,
  ...
  if (cell != null) {
    ...
    // 外层容器
    let myModalContainer = this.createDiv("myModalContainer");

    // 标题区域
    let titleArea = this.createDiv("title");
    titleArea.innerHTML = "关联菜单列表";

    // 搜索区域
    let searchArea = this.createDiv("searchArea");
    //输入框
    let inputEle = document.createElement("input");
    inputEle.type = "text";
    inputEle.id = "inputText";
    inputEle.value = "";
    inputEle.placeholder = "请输入搜索内容";

    // 按钮
    let btnEle = document.createElement("button");
    btnEle.innerHTML = "搜&nbsp;&nbsp;索";
    searchArea.appendChild(inputEle);
    searchArea.appendChild(btnEle);
    ...
    // 全选
    let checkAllBtn = document.createElement("input");
    checkAllBtn.type = "checkbox";
    checkAllBtn.id = "checkAll";

    thCol.appendChild(checkAllBtn);
    thRaw.appendChild(thCol);
    // 循环生成表头
    let columnOptions = [
      {
        title: "编号",
    ...
    columnOptions.forEach((el) => {
      ...
      domTH.innerHTML = el.title;
      thRaw.appendChild(domTH);
    });
    thead.appendChild(thRaw);
    ...
    if (dataList.length) {
      ...
      // 表格列中第一个为checkbox元素
      let btnTrCheckAll = document.createElement("input");
      btnTrCheckAll.type = "checkbox";
      col.appendChild(btnTrCheckAll);
      raw.appendChild(col);

      dataList.forEach((item, idx) => {
        // 表格行
        let tempTr = document.createElement("tr");
        // tempTd.innerHTML = "暂无数据~";
        // tbody.append(emptyDom);
        columnOptions.forEach((colItem, colIdx) => {
          let tempTd = document.createElement("td");
          tempTd.innerHTML = item[colItem.dataIndex];
          tempTr.appendChild(tempTd);
        });
        // tbody.appendChild(tempTr);
      });
    } else {
      // let emptyDom = document.createElement("tr");
    ...
    // 底部按钮
    let footerAction = this.createDiv("footerBtns");
    let cancelBtn = document.createElement("button");
    cancelBtn.innerHTML = "关&nbsp;&nbsp;闭";
    let confirmBtn = document.createElement("button");
    confirmBtn.innerHTML = "保&nbsp;&nbsp;存";

    footerAction.appendChild(cancelBtn);
    footerAction.appendChild(confirmBtn);

    myModalContainer.appendChild(titleArea);
    myModalContainer.appendChild(searchArea);
    myModalContainer.appendChild(tableArea);
    if (!dataList.length) {
      let emptyDom = this.createDiv("emptyDom");
      emptyDom.innerHTML = "暂无数据~";
      myModalContainer.appendChild(emptyDom);
    } else {
      // 遍历数据
    }
    // myModalContainer.appendChild(footerAction);底部按钮

    this.showDialog(myModalContainer, 1000, 420, true, true, null, false);
    dlg.init();
  }
};
...
EditorUi.prototype.createKeyHandler = function (editor) {
  ...
  if (!this.editor.chromeless || this.editor.editable) {
    ...
    keyHandler.bindAction(79, true, "relationOrder"); // Ctrl+O
