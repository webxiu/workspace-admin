...
import { getHolidays, updateStartDate, getHolidaysByOneDate } from "./utils";
...
const setStartDate = (row) => {
  updateStartDate(holidaySettingList.value, row);
  const { holidays, endDate } = getHolidays(holidaySettingList.value, row.start, +row.duration);
  row.end = endDate;
};
