...
import { ElMessage, ElMessageBox, FormRules, UploadProps, dayjs } from "element-plus";
import {
  PayslipDataItemType,
...
export const useConfig = () => {
  ...
  const getColumnConfig = async () => {
    ...
    const signRenderer: RendererType = ({ row }) => {
      return (
        <div class="flex-center">
          <el-image
            src={row.signatureFilePath}
            preview-src-list={[row.signatureFilePath]}
    ...
    const { columnArrs, buttonArrs } = await getMenuColumns([{ Status: statusRenderer, signatureFilePath: signRenderer }]);
    columnArr.value = columnArrs;
    const [data] = columnArrs;
    if (data?.length) columnData = data;
    updateButtonList(buttonList, buttonArrs[0]);
    // 导入工资条测试代码
    if (location.host !== "app.deogra.com") {
      buttonList.value.push({
        ...
        disabled: false,
        isDropDown: true,
        uploadProp: { action: "#", accept: ".", autoUpload: false, onChange: onUploadPayslip }
      });
    }
  ...
  const onExport2 = () => {
    ...
    exportImgToExcel(
      ...
      { imgProp: "signatureFilePath", imgSize: [60, 90], fileName: `【${typeName}】${formData.gzDate}月工资条` }
    );
  };

  const onView = (row) => {
    const rowData = row ?? selectionList.value[0];
    if (!rowData) return message.error("请选择记录");
    router.push(`/financeDept/payrollMgmt/payslip/detail?payslipId=${rowData.Id}&gzmbNo=${formData.gzmbNo}&gzmbb=${formData.gzmbb}&gzDate=${formData.gzDate}`);
  };
  ...
  const onTemplateConfig = () => {
    router.push(`/financeDept/payrollMgmt/payslip/templateConfig?gzmbNo=${formData.gzmbNo}&gzmbb=${formData.gzmbb}`);
  };
  ...
  const onUploadPayslip: UploadProps["onChange"] = (uploadFile) => {
    ...
    const { gzDate, gzmbb, gzmbNo } = formData;
    const param = { gzDate, gzmbb, gzmbNo };
    const fd = new FormData();
    fd.append("file", rawFile);
    fd.append("data", JSON.stringify(param));
    HxUploadProgress({ uploadApi: importPayslipDataInfo2, fd }).then(({ data }) => {
      console.log("上传结果", data);
    });
  };

  function onPrint() {
    if (!rowsData.value.length) return message.error("请勾选打印项目");
    const formRef = ref();
