...
import { fetchMeetPayMoneyList, fetchOtherPayMoneyList, fetchPrePayMoneyList } from "@/api/oaManage/financeDept";
...
defineOptions({ name: "FinanceDeptFinanceBIMeetIndex" });
...
const columns = ref<ColumnsDataType[]>([
  { label: "供应商名称", prop: "FNAME", width: 400 },
  { label: "期初余额", prop: "FBeginBalance" },
  { label: "已付", prop: "FDEBIT" },
  { label: "应付", prop: "FCREDIT" },
  { label: "期末余额", prop: "FEndBalance" }
]);
...
const formatterVal = (row, column) => {
  if (/(FBeginBalance|FDEBIT|FCREDIT|FEndBalance)/.test(column.property)) {
    return fixed2AndAddcomma(row[column.property]);
  }
