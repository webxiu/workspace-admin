...
const setChart1 = async (list, resCols) => {
  const filterInitArr = list.filter((item) => item.ItemName === "成本损耗比例");
  saleDataTableRef.value?.setDataList(filterInitArr, "rate", resCols);
...
const fresh = () => {
  myChart.value = markRaw(echarts.init(document.getElementById("productPercent")));
...
onMounted(() => {
  const myCharts1 = document.getElementById("productPercent");
...
<template>
  <div class="duration">
    <div class="sale">
      <div id="productPercent" style="width: 100%; height: 421px; margin: auto" />
      <div class="sd-table"><SaleFreeTable ref="saleDataTableRef" :point="true" /></div>
    </div>
  </div>
</template>
