...
const setChart1 = async (list, resCols) => {
  const filterInitArr = list.filter((item) => item.ItemName === "产品平均工资");
...
const fresh = () => {
  myChart.value = markRaw(echarts.init(document.getElementById("userMoneyId")));
...
onMounted(() => {
  const myCharts1 = document.getElementById("userMoneyId");
...
<template>
  <div class="duration">
    <div class="sale">
      <div id="userMoneyId" style="width: 100%; height: 421px; margin: auto" />
