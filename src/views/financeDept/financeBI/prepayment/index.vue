...
import { fetchPrePayMoneyList } from "@/api/oaManage/financeDept";
...
defineOptions({ name: "FinanceDeptFinanceBIPrepaymentIndex" });
...
const columns = ref<ColumnsDataType[]>([
  { label: "供应商", prop: "FNAME", width: 400 },
...
const formatterVal = (row, column) => {
  if (column.property === "FEndBalance") {
    return fixed2AndAddcomma(row.FEndBalance);
  }
