...
export const useConfig = () => {
    ...
    FormRef.value.getRef().then(({ valid, sendList, formData, detailData, data }) => {
      ...
      if (valid) {
        ...
        if (!sendList.value?.length) {
          return message.error("抄送人清单不能为空");
        }
  ...
  const onReceiveBill = (row: CustomerComplaintItemType) => {
    if (row.marketState !== AuditState.submited) {
      return message.error("审核状态不是已提交，不能接收！(需要业务员在修改页面提交单据)");
    }
  ...
  const onSubmit = wrapFn(rowData, ({ text }) => {
    ...
    if (row.marketState !== AuditState.reply) {
      return message.error("请接收并回复单据后，再提交审批");
  ...
  const onRevoke = wrapFn(rowData, ({ text }) => {
    ...
    if (row.marketState === AuditState.submited) {
      ...
      message.error("单据已被品质工程师接收，无法主动撤回，请联系品质工程师对单据进行【退回单据】操作。");
    } else {
      message.error("只能对审核状态为【已提交】的客诉单进行撤回！");
    }
  });
  ...
  const onBack = wrapFn(rowData, ({ text }) => {
    ...
    if ([BillState.reject].includes(row.state)) {
      ...
      if ([AuditState.receive, AuditState.reply].includes(row.marketState)) {
        ...
        message.error("只能对审核状态为【已接收】或【已回复】的客诉单进行退回！");
      }
    } else {
      message.error("只能对单据状态为【待提交】或【退回重审】的客诉单进行退回！");
    }
  });
