...
import { combineArrays } from "@/views/marketing/saleManage/quoteApply/utils/hook";
...
function onCalculatePrice(row) {
  if ([PurchaseState.submit, PurchaseState.audit, PurchaseState.success].includes(row.verificationState)) {
    return message.error("当前状态不能提交");
  }
  if (!row.materialCode) return message.error("物料编码不存在");
  submitQuoteSale(row).then(({ data }) => {
    if (!data) return message.error("发送失败");
...
function onOperate(type: "material" | "package", action: "clear" | "add" | "delete" | "import") {
  if (!props.isEdit && action === "import") return message.error("当前状态不能操作");
  ...
  if (action === "delete") {
    if (!_row?.value) return message.error("请选择要删除的行");
...
function onUploadChange(type: "material" | "package") {
  if (!detailInfo.value.id) return message.error("详情数据不存在");
  regenerateQuoteSale(detailInfo.value)
    .then(({ data }) => {
      if (!data) return message.error("重新导入失败");
