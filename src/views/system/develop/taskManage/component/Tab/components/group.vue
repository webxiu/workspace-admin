<template>
  <div class="group" :style="{ 'background-color': titleGroup[type].bgColor }">
      ...
      <div class="left">
        <div class="title-btn">
          <el-button :color="titleGroup[type].color" type="primary" size="small" :icon="titleGroup[type].icon"
            ><span :class="{ txt: true, 'white-txt': type !== '1' }">{{ titleGroup[type].title + " " + total }}</span>
          </el-button>
        </div>
      </div>
    </div>
    <div
      ...
      @mouseover.stop="item.showFloatPop = true"
      @mouseleave.stop="item.showFloatPop = false"
      v-for="(item, idx) in todoList"
      :key="idx"
    >
      <div class="task">
        <span v-if="!item.editTaskNameFlag || type !== '1'" @click="item.editTaskNameFlag = true">{{ item.taskName }}</span>
        <span v-else>
          <el-input placeholder="请输入任务名称" v-model="item.taskName" size="small" @blur="item.editTaskNameFlag = false" />
        </span>
      </div>
      <div class="right-pop" v-if="item.showFloatPop && type !== '3'" aria-hidden>
        <div class="icon-list">
          <div class="icon-item">
            <el-icon @click="() => clickCheck(item)"><Check /></el-icon>
          </div>
          <div class="icon-item" style="margin: 0 8px" v-if="type === '1'">
            <el-icon>
              <Plus v-if="!subtaskRef[idx].showAdd" @click.stop="() => addSubtask(idx)" />
              <Close v-else @click.stop="() => (subtaskRef[idx].showAdd = false)" />
            </el-icon>
          </div>
          <div class="icon-item" @click="() => delItem(item, idx)" v-if="type === '1'">
      ...
      <el-popover
        :disabled="type !== '1'"
        v-model:visible="item.visiblePop2"
        :show-arrow="false"
        placement="bottom-start"
        :offset="4"
        :width="250"
      ...
      <subtask ref="subtaskRef" :arrowRight="item.arrowRight" :childrenList="item.children" :type="type" />
    </div>
    ...
    <div class="center" v-if="showAdd">
      ...
      <el-popover v-model:visible="visibleFdPop" class="user-pop" :show-arrow="false" placement="bottom-start" :offset="4" :width="260" trigger="click">
        ...
        <template #default>
          ...
          <div class="line"><el-divider style="margin: 8px 0" /></div>
          <div class="user-list">
            <div class="user-item" v-for="(item, idx) in nameOptions" :key="idx" @click="() => clickNameFd(item)">
              <div class="avatar">
                <el-avatar :size="20" :src="item.avatar" />
              </div>
              <div class="name">{{ item.label || "指派人员" }}</div>
            </div>
          </div>
        </template>
      </el-popover>
  ...
  <div style="display: flex; justify-content: center; height: 40px; line-height: 40px">
    <el-pagination hide-on-single-page layout="prev, pager, next" :total="total" @current-change="currentChange" />
  </div>
</template>
...
import { addMainTask, deleteTask, taskManageList, updateTaskStatus } from "@/api/systemManage";
import { cloneDeep } from "@pureadmin/utils";

const props = defineProps(["type", "nameOptions", "searchName", "changeUserName"]);

const titleGroup = {
  1: { title: "未开始", color: "#ddd", bgColor: "#f3f3f5", icon: Opportunity },
  2: { title: "进行中", color: "#0880ea", bgColor: "#f2f8fe", icon: Opportunity },
  5: { title: "待确认", color: "#E6A23C", bgColor: "rgb(252.5, 245.7, 235.5)", icon: CircleCheckFilled },
  3: { title: "已完成", color: "#299764", bgColor: "#f4faf7", icon: CircleCheckFilled }
};
...
watch(
  props,
  (newVal) => {
    userList.value = newVal.nameOptions.map((item) => ({ ...item, avatar: "https://fuss10.elemecdn.com/e/5d/4a731a90594a4af544c0c25941171jpeg.jpeg" }));
  },
    ...
    .then(() => {
      updateTaskStatus({ ...row, newStatus }).then(({ data }) => {
        if (data) {
          ...
          if (cb) cb();
          props.changeUserName(props.searchName);
        }
      });
    })
    .catch(() => {});
...
const clickCheck = (row) => {
  switch (props.type) {
    case "1":
      commonActionFn("2", "开始", row);
      return;
    case "2":
      commonActionFn("5", "提交", row);
      return;
    case "5":
      commonActionFn("3", "完成", row);
      return;
    default:
      return;
  }
...
const currentChange = (val) => {
  ...
  taskManageList({
    ...
    hideChildDone: true,
    select: props.type,
    responsibleUserName: props.searchName
  }).then((res: any) => {
    if (res) {
...
const delItem = (item, idx) => {
  commonActionFn("4", "删除", item, () => {
    deleteTask({ id: item.id, parentId: item.parentId }).then((res) => {
      if (res.data) {
...
const addMainTaskAction = () => {
  showAdd.value = true;
  if (subtaskRef.value) {
    // subtaskRef.value.showAdd = false;
    subtaskRef.value.forEach((el) => (el.showAdd = false));
  }
};
...
const onSaveFd = () => {
  ...
  const reqParams = {
    ...
    startTime: formData.value.endDate,
    endTime: formData.value.endDate,
    responsibleUserCode: formData.value.curUserCode,
    priority: formData.value.curFlagName,
    number: 1,
    file: "",
    taskTypeCode: "",
    parentId: 0,
    taskContent: "",
    score: 0,
    responsibleUserName: formData.value.curUserName
  };

  const fd = new FormData();
  fd.append("param", JSON.stringify(reqParams));

  showMessageBox(`确认要新增吗?`)
    .then(() => {
      addMainTask(fd).then((res) => {
...
const clickName = (item, idx) => {
  ...
  todoList.value[idx].responsibleUserName = item.label;
  todoList.value[idx].responsibleUserCode = item.userCode;
};

const clickNameFd = (item) => {
  formData.value.curUserName = item.label;
  formData.value.curUserCode = item.userCode;
