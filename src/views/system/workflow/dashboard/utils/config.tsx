...
// 编辑SQL单据表单
export const formConfigs = ({ dbKeyList }): FormConfigItemType[] => {
  return [
    {
      label: "标题",
      prop: "title",
...
// 查看SQL单据表单
export const lookFormConfigs = ({ dbKeyList }): FormConfigItemType[] => {
  return [
    {
      label: "标题",
      prop: "title",
    ...
    },
    {
      ...
      colProp: { span: 24 },
      render: ({ formModel, row }) => {
        const value = Number(formModel[row.prop]) === 1 ? "已执行" : "未执行";
        return <el-input v-model={value} disabled />;
      }
    },
    {
      ...
      colProp: { span: 24 },
      render: ({ formModel, row }) => {
        const value = Number(formModel[row.prop]) === 1 ? "是" : "否";
        return <el-input v-model={value} disabled />;
      }
    },
    {
