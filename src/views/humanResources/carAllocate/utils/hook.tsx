...
export const useConfig = () => {
  ...
  const onPrint = () => {
    ...
    router.push("/humanResources/carAllocate/print?id=" + currentRow.value.id + "&billNo=" + currentRow.value.billNo);
  };
  ...
  const openDialog = async (type: string, row) => {
    const title = { add: "修改", edit: "修改" }[type];
    const formRef = ref();
    ...
    addDialog({
      title: `${title}`,
      props: { id: row?.id, type: type, formInline: _formData },
      width: "1050px",
      draggable: true,
      fullscreenIcon: true,
      okButtonText: "保存",
