...
export const useConfig = () => {
  ...
  const onUploadChange: UploadProps["onChange"] = (uploadFile) => {
    ...
    if (!["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-excel"].includes(rawFile.type)) {
      return message.warning("文件必须为xls或xlsx格式!");
    }
  ...
  const onBatchDelete = () => {
    ...
    if (!rows?.length) return message.error("请选择要删除的内容");
  ...
  const onGenerateAttSummary = () => {
    if (!formData.date) return message.error("请选择要生成的日期");

    showMessageBox(`确认要生成【${formData.date}】的数据吗?`).then(() => {
      generateAttSummary({ yearMonth: formData.date })
        .then((res) => {
          if (res.data) {
  ...
  const buttonList = ref<ButtonItemType[]>([
    ...
    { clickHandler: onGenerateAttSummary, type: "default", text: "生成考勤汇总", icon: Download, isDropDown: true },
