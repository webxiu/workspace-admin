...
import { fetchCommissionSheetDetail, fetchCommissionSheetList } from "@/api/oaManage/humanResources";
...
const custmRender = (): Record<string, RendererType> => {
  return {
    ...
    commissionAmount: (data) => editCellRender({ data, isEdit: false })
  };
};
...
const initData = () => {
  if (props.rowData?.billNo) {
    loading.value = true;
    const p1 = fetchCommissionSheetList({ billNo: props.rowData?.billNo, page: 1, limit: 30 });
    const p2 = fetchCommissionSheetDetail({ id: props.rowData?.id });
    Promise.all([p1, p2])
      .then((res: any) => {
        if (res[0].data) {
