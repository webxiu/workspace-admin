...
import { getQAInstoreRateData } from "@/api/oaManage/productMkCenter";
...
defineOptions({ name: "ProductMkCenterQualityQualityBIInstoreRateIndex" });
...
const calcChartTitle = (selectDay, num) => {
  ...
  if (num === 2) return dateStr + "来料合格批次与总来料批次(单位：笔)";
  if (num === 1) return dateStr + "来料合格率(单位：%)";
};
...
const getChartData = async (myChart, myChart1?) => {
  ...
  console.log(buttonArrs[0], "buttonArrs[0]");
  getQAInstoreRateData({
    fbilltype: "1",
