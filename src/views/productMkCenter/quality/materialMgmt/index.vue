      ...
      <PureTableBar :columns="columns" @refresh="onFresh" style="padding-top: 0" @change-column="setUserMenuColumns">
        ...
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            ...
            <template #samplingPlan="{ row }">
              {{ row.materialOtherVO?.fIncSampSchemeId ?? "" }}
            </template>
            <template #qualityCheckPlan="{ row }">
              {{ row.materialOtherVO?.fIncQcSchemeId ?? "" }}
            </template>
            <template #stockReCheck="{ row }">
              {{ row.materialOtherVO?.fEnableCyclistQCSTK ?? "" }}
            </template>
            <template #stockReCheckNotice="{ row }">
              {{ row.materialOtherVO?.fEnableCyclistQCSTKEW ?? "" }}
            </template>
          </pure-table>
        </template>
      </PureTableBar>
...
defineOptions({ name: "ProductMkCenterQualityMaterialMgmtIndex" });
