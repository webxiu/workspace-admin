...
import { getProductivityChartData, getQAInstoreRateData } from "@/api/oaManage/productMkCenter";
...
defineOptions({ name: "ProductMkCenterProductDeptProductivityIndex" });
...
const calcChartTitle = (selectDay, num) => {
  ...
  if (num === 2) return dateStr + "标准工时与员工工时(单位：H)";
  if (num === 1) return dateStr + "生产效率(百分比：%)";
};
  ...
  getProductivityChartData({
    ...
    .then((res: any) => {
      ...
      if (res.data) {
        ...
        option.series[0].data = findChartItem(res, "实际标准工时");
        option.series[1].data = findChartItem(res, "员工总工时");
        option1.series[0].data = findChartItem(res, "生产效率", 1);
