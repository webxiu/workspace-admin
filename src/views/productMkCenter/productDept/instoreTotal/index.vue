...
import { getProductInstoreTotalChartData, getProductPlanRateChartData } from "@/api/oaManage/productMkCenter";
...
defineOptions({ name: "ProductMkCenterProductDeptInstoreTotalIndex" });
...
const calcChartTitle = (selectDay, num) => {
  ...
  if (num === 1) return dateStr + "生产入库数量(单位：Pcs)";
};
...
const getChartData = async (myChart1) => {
  ...
  getProductInstoreTotalChartData({
    ...
    .then((res: any) => {
      ...
      if (res.data) {
        ...
        option1.series[0].data = findChartItem(res, "数量", 1);
...
<template>
  <div class="chart-outer" v-loading="loading">
    ...
    <div :style="{ height: maxHeight + 'px' }">
      <div id="deliveryRate1" style="height: 450px" />
