...
export const useConfig = () => {
  ...
  function onUpdateIndex() {
    ...
    sortEsopStation({ id: formData.id, workStationDTOs }).then(({ data }) => {
      if (!data) return message.error("排序失败");
  ...
  function onAdd(type: MulHandleType) {
    if (type !== "sort") {
      if (!rowData.value) return message.error("请选择排位");
    }
    if (type === "sort") {
    ...
    if (type === "check") {
      if (rowData.value.checkRuleVOS.length >= 2) {
        return message.error("最多添加2条检测记录");
      }
    ...
    if (type === "tool") {
      const toolLimit = 2;
      if (!rowData3.value) return message.error("请选择检测记录再添加");
      if (rowData.value.checkRuleVOS[toolIndex.value]?.toolParametersVOS.length >= toolLimit) {
        return message.error(`最多添加${toolLimit}条工具参数`);
      }
  ...
  function onDelete(type: MulHandleType) {
    ...
    showMessageBox(`确认要删除选中${_title}吗? ${_tip}`).then(() => {
      ...
      if (type === "material") {
        if (!rowData2.value) return message.error("请选择物料");
        rowData.value.materialVOS = rowData.value.materialVOS.filter((f) => f.id !== rowData2.value.id);
      }
      if (type === "check") {
        if (!rowData3.value) return message.error("请选择检测记录");
      ...
      if (type === "tool") {
        if (!rowData4.value) return message.error("请选择工具参数");
  ...
  function onPrint() {
    if (!rowData.value) {
      return message.error("请选择一条排位表数据");
    }
    const formRef = ref();
