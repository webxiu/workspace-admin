...
export const useTable = () => {
  ...
  const getConfig = async () => {
    let columnData: TableColumnList[] = [
      ...
      { label: "抽样方案", prop: "samplingPlan", slot: "samplingPlan" },
      { label: "质检方案", prop: "qualityCheckPlan", slot: "qualityCheckPlan" },
      { label: "库存周期复检", prop: "stockReCheck", slot: "stockReCheck" },
      // { label: "复检周期(天)", prop: "reCheckDays" },
      { label: "库存周期复检提醒", prop: "stockReCheckNotice", slot: "stockReCheckNotice" },
      // { label: "提醒提前期(天)", prop: "stockReCheckNoticeDays" },
      { label: "模号", prop: "model" },
  ...
  function openEditDialog(row) {
    ...
    const stockCycleCheck = ref([]);
    const cycleCheckNotice = ref([]);
    ...
    const _formData = {
      ...
      samplingPlan: row?.materialOtherVO?.fIncSampSchemeId,
      qualityCheckPlan: row?.materialOtherVO?.fIncQcSchemeId,
      stockReCheck: row?.materialOtherVO?.fEnableCyclistQCSTK,
      // reCheckDays: row?.materialOtherVO?.reCheckDays,
      stockReCheckNotice: row?.materialOtherVO?.fEnableCyclistQCSTKEW
      // stockReCheckNoticeDays: row?.materialOtherVO?.stockReCheckNoticeDays
    };
    console.log(_formData, "_formData..");
    getBOMTableRowSelectOptions({ optioncode: "MaterialUnits,StockCycleCheck,CycleCheckNotice" }).then((res) => {
      if (res.data) {
        const findRes = res.data.find((item) => item.optionCode === "MaterialUnits")?.optionList || [];
        bomUnitOpts.value = findRes;

        const findRes1 = res.data.find((item) => item.optionCode === "StockCycleCheck")?.optionList || [];
        stockCycleCheck.value = findRes1;

        const findRes2 = res.data.find((item) => item.optionCode === "CycleCheckNotice")?.optionList || [];
        cycleCheckNotice.value = findRes2;
      }
    });
    ...
    addDialog({
      ...
      props: {
        ...
        formRules,
        formConfigs: formConfigs({ selectOpts: { bomUnitOpts, stockCycleCheck, cycleCheckNotice } })
  ...
  const onSubmitGroup = (data, callback: Function) => {
    const updateParams = {
      ...
      materialOtherDTO: {
        ...
        materialId: data.id,
        fIncSampSchemeId: data.samplingPlan,
        fIncQcSchemeId: data.qualityCheckPlan,
        fEnableCyclistQCSTK: data.stockReCheck,
        fEnableCyclistQCSTKEW: data.stockReCheckNotice
      }
    };
