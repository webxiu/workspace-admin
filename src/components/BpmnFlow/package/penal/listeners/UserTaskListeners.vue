    ...
    <el-table :data="elementListenersList" size="small" border>
      ...
      <el-table-column label="操作" width="100px">
        <template #default="{ row, $index }">
          ...
          <el-button size="small" type="danger" link @click="removeListener(row, $index)">移除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-drawer v-model="listenerFormModelVisible" title="任务监听器" :size="`${width}px`" append-to-body destroy-on-close>
      <el-form size="small" :model="listenerForm" label-width="96px" ref="listenerFormRef" @submit.prevent>
        <el-form-item label="事件类型" prop="event" :rules="{ required: true, trigger: ['blur', 'change'] }">
          <el-select v-model="listenerForm.event" placeholder="必填">
        ...
        <el-form-item label="监听器类型" prop="listenerType" :rules="{ required: true, trigger: ['blur', 'change'] }">
          <el-select v-model="listenerForm.listenerType" placeholder="必填">
        ...
        >
          <el-input v-model="listenerForm.class" clearable placeholder="必填" />
        </el-form-item>
        <el-form-item
          ...
          :rules="{ required: true, trigger: ['blur', 'change'] }"
        >
          <el-input v-model="listenerForm.expression" clearable placeholder="必填" />
        </el-form-item>
        <el-form-item
          ...
          :rules="{ required: true, trigger: ['blur', 'change'] }"
        >
          <el-input v-model="listenerForm.delegateExpression" clearable placeholder="必填" />
        </el-form-item>
        <template v-if="listenerForm.listenerType === 'scriptListener'">
          ...
          >
            <el-input v-model="listenerForm.scriptFormat" clearable placeholder="必填" />
          </el-form-item>
          <el-form-item
            label="脚本类型"
            prop="scriptType"
          ...
          >
            <el-select v-model="listenerForm.scriptType" placeholder="必填">
          ...
          >
            <el-input v-model="listenerForm.value" clearable placeholder="必填" />
          </el-form-item>
          <el-form-item
      ...
      <el-table :data="fieldsListOfListener" size="small" max-height="240" border fit style="flex: none">
        ...
        <el-table-column label="操作" width="100px">
          <template #default="{ row, $index }">
            <el-button size="small" link type="primary" @click="openListenerFieldForm(row, $index)">编辑</el-button>
            <el-divider direction="vertical" />
            <el-button size="small" link type="danger" @click="removeListenerField(row, $index)">移除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
const resetListenersList = () => {
  ...
  bpmnElementListeners.value = bpmnElement.value.businessObject?.extensionElements?.values?.filter((ex) => ex.$type === `${prefix}:TaskListener`) ?? [];
  elementListenersList.value = bpmnElementListeners.value.map((listener) => initListenerType(listener));
  setTaskForm(bpmnElement.value as any, { elementListenersList: elementListenersList.value });
};
const openListenerForm = (listener?, index = -1) => {
  if (listener) {
